...
class AssignWebcamController extends Controller
{
    public function index()
    {
        $webcams = Webcam::where('status','Y')->get();
        return view('assignwebcams.index',compact('webcams'));
    }
    ...
    public function store(Request $request)
    {
         ...
         $assignprot = 1;
         return view('assignwebcams.create',compact('assignprot','productids','locations','campus','departments','offices','vendors','makes','models'));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        { 
            ...
            $serial_number = Webcam::where('id', $val)->value('serial_number');
            ...
            .substr(strtoupper($product->office), 0, 3).'/WEB0'.$val;
          ...
          "employee" => $input['employee'],"employeeid" => $input['employeeid'],"asset_type" => 'Webcam',"serial_number" => $serial_number,
          "comment" => $input['comment'],"updated_at" => now()]);

          $webcams = Webcam::where('id',$val)->update(["status" => 'N']);

         // $product->save();
        }
        return redirect()->route('assignwebcams.index')->with('message','Product  has been updated successfully.');
    }
