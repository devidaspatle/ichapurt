...
class AssignWifidiviceController extends Controller
{
    public function index()
    {
        $wifidivices = Wifidivice::where('status','Y')->get();
        return view('assignwifidivices.index',compact('wifidivices'));
    }
    ...
    public function store(Request $request)
    {
         ...
         $assignprot = 1;
         return view('assignwifidivices.create',compact('assignprot','productids','locations','campus','departments','offices','vendors','makes','models'));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        { 
            ...
            $serial_number = Wifidivice::where('id', $val)->value('serial_number');
            ...
            .substr(strtoupper($product->office), 0, 3).'/WIF0'.$val;
          ...
          "employee" => $input['employee'],"employeeid" => $input['employeeid'],"asset_type" => 'Wifidivice',"serial_number" => $serial_number,
          "comment" => $input['comment'],"updated_at" => now()]);

          $wifidivices = Wifidivice::where('id',$val)->update(["status" => 'N']);

         // $product->save();
        }
        return redirect()->route('assignwifidivices.index')->with('message','Product  has been updated successfully.');
    }
