...
class AssignprojectorController extends Controller
{
    public function index()
    {
        $projectors = Projector::where('status','Y')->get();
        return view('assignprojectors.index',compact('projectors'));
    }
    ...
    public function store(Request $request)
    {
         ...
         $assignprot = 1;
         return view('assignprojectors.create',compact('assignprot','productids','locations','campus','departments','offices','vendors','makes','models'));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        { 
            ...
            $serial_number = Projector::where('id', $val)->value('serial_number');
            ...
            .substr(strtoupper($product->office), 0, 3).'/PRO0'.$val;
          ...
          "employee" => $input['employee'],"employeeid" => $input['employeeid'],"asset_type" => 'Projector',"serial_number" => $serial_number,
          "comment" => $input['comment'],"updated_at" => now()]);

          $projectors = Projector::where('id',$val)->update(["status" => 'N']);

         // $product->save();
        }
        return redirect()->route('assignprojectors.index')->with('message','Product  has been updated successfully.');
    }
