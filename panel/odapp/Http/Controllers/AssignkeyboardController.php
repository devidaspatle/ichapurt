...
class AssignkeyboardController extends Controller
{
    public function index()
    {
        $keyboards = Keyboard::where('status','Y')->get();
        return view('assignkeyboards.index',compact('keyboards'));
    }
    ...
    public function store(Request $request)
    {
         ...
         $assignprot = 1;
         return view('assignkeyboards.create',compact('assignprot','productids','locations','campus','departments','offices','vendors','makes','models'));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        { 
            ...
            $serial_number = Keyboard::where('id', $val)->value('serial_number');
            ...
            .substr(strtoupper($product->office), 0, 3).'/KEY0'.$val;
          ...
          "employee" => $input['employee'],"employeeid" => $input['employeeid'],"asset_type" => 'Keyboard',"serial_number" => $serial_number,
          "comment" => $input['comment'],"updated_at" => now()]);

          $keyboards = Keyboard::where('id',$val)->update(["status" => 'N']);

         // $product->save();
        }
        return redirect()->route('assignkeyboards.index')->with('message','Product  has been updated successfully.');
    }
