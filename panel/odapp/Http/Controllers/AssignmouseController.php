...
class AssignmouseController extends Controller
{
    public function index()
    {
        $mouses = Mouse::where('status','Y')->get();
        return view('assignmouses.index',compact('mouses'));
    }
    ...
    public function store(Request $request)
    {
         ...
         $assignprot = 1;
         return view('assignmouses.create',compact('assignprot','productids','locations','campus','departments','offices','vendors','makes','models'));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        { 
            ...
            $serial_number = Mouse::where('id', $val)->value('serial_number');
            ...
            .substr(strtoupper($product->office), 0, 3).'/MOU0'.$val;
          ...
          "employee" => $input['employee'],"employeeid" => $input['employeeid'],"asset_type" => 'Mouse',"serial_number" => $serial_number,
          "comment" => $input['comment'],"updated_at" => now()]);

          $mouses = Mouse::where('id',$val)->update(["status" => 'N']);

         // $product->save();
        }
        return redirect()->route('assignmouses.index')->with('message','Product  has been updated successfully.');
    }
